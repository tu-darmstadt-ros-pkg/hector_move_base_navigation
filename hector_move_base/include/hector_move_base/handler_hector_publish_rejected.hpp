#ifndef HANDLER_HECTOR_PUBLISH_REJECTED_H_
#define HANDLER_HECTOR_PUBLISH_REJECTED_H_

#include <hector_nav_core/hector_move_base_handler.h>

namespace hector_move_base_handler {

class HectorPublishRejectedHandler : public HectorMoveBaseHandler {
private:
    ros::Publisher result_pub_;
public:
    HectorPublishRejectedHandler(hector_move_base::IHectorMoveBase* interface) : HectorMoveBaseHandler(interface){
        ros::NodeHandle private_nh("~");
        result_pub_ = private_nh.advertise<hector_move_base_msgs::MoveBaseActionResult>("result", 0 );
    }

    hector_move_base::RESULT handle(){
        ROS_DEBUG("[publish_rejected_handler]: starting publish rejected");
        ...
        action_result.status.status = actionlib_msgs::GoalStatus::REJECTED;
