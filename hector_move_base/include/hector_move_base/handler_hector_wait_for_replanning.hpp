...
public:
    ...
    bool wait4ReplanTimeExceeded()
    {
        return hectorMoveBaseInterface->getCurrentActionPath().goal.target_path.header.stamp + ros::Duration(max_time_to_replan_) < ros::Time::now();
    }

    hector_move_base::RESULT handle()
    {
        if (wait4ReplanTimeExceeded())
        {
            ROS_DEBUG("[hector_move_base] [wait_for_replanning_handler] ALTERNATIVE start replanning");
